...
import os
from networks.ner_bilstm import NERModel
import numpy as np
...
def main():
    ...
    dictionary, embeddings = util.generate_embeddings([lexicon_ner_dic, lexikon_val_ner_dic])
    ...
    ner = NERModel(embeddings, len(classes_ner), util)
    # inv_classes_ner = {idx: tag for tag, idx in classes_ner.items()}
    inv_classes_ner = {idx: tag for tag, idx in classes_ner.items()}
    ner.build()
    ner.train(dev_ner, val_ner, classes_ner)
