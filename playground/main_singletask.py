...
def main():
    ...
    dictionary, embeddings = util.generate_embeddings([lexicon_pos_dic, lexikon_val_pos_dic])
    ...
    pos = POSModel(embeddings, len(classes_pos), util)
    # inv_classes_ner = {idx: tag for tag, idx in classes_ner.items()}
    inv_classes_ner = {idx: tag for tag, idx in classes_pos.items()}
    pos.build()
    start = time.time()
    pos.train(dev_pos, val_pos, classes_pos)
